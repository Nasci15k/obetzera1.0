    ...
    public function requestQrcodeOnda($request)
    {
        try {
            ...
            $response = Http::withHeaders([
                ...
                // "split"       => [ 
                //      "email"      => 'seuemail@gmail.com', // Email do recebedor do split
    ...
    public function pixCashOutOnda($id, $tipo)
    {
        ...
        if ($withdrawal != null) {
            ...
            switch ($withdrawal->pix_type) {
                case 'document':
                    $tipoPix = strlen($withdrawal->pix_key) > 14 ? "cnpj" : "cpf";
                    // remove pontos, traços e barras
                    $key = preg_replace('/[^0-9]/', '', $withdrawal->pix_key);
                    break;

                case 'phoneNumber':
                    $key = "+55" . preg_replace('/[^0-9]/', '', $withdrawal->pix_key);
                    $tipoPix = "phone";
                    break;
            ...
            $response = Http::withOptions([
                ...
                "Authorization" => "Bearer " . $token['acessToken']
            ])->post(self::$uriOnda . '/api/v1/withdraw/pix', $payload);

            // logs para conferência
            Log::info('[ONDAPAY] Payload de saque', $payload);
            Log::info('[ONDAPAY] Resposta', $response->json());
